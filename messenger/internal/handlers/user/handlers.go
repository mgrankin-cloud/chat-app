...
func (s *serverAPI) GetUser(
	...
	email, username, phone, photo, active, err := s.user.GetUser(ctx, request.GetUserId())
	if err != nil {
...
func (s *serverAPI) UpdateUser(
	...
	success, err := s.user.UpdateUser(ctx, request.GetUserId(), request.GetEmail(), request.GetUsername(), request.GetPhoto(), request.GetActive())
	if err != nil {
...
func (s *serverAPI) DeactivateUser(ctx context.Context, request *ssov2.GetUserRequest) (*ssov2.GetUserResponse, error) {
	if request.GetUserId() == 0 {
		return nil, status.Error(codes.InvalidArgument, "user id is required")
	}

	err := s.storage.updateUserActiveStatus(ctx, request.GetUserId(), false)
	if err != nil {
	...
	email, username, phone, photo, active, err := s.user.GetUser(ctx, request.GetUserId())
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to get user after deactivation: %v", err)
	}
